...
| 7 | CI workflow `.github/workflows/student-5.yml` | TL-NFR-05 | **Complete in source; run evidence outstanding** | Two separate pytest steps, `docker compose config --quiet`, three image builds; step-by-step description and pipeline diagram in `testing-evidence.md` and `architecture.md`. Every step has a local capture (`pytest-database.txt`, `pytest-backend.txt`, `compose-build.txt`), but a green run on GitHub itself is still uncaptured. Commits `fc00e4a`, `9e894c9`, PR #40. **TODO: attach CI run screenshot or URL** from the Actions tab, workflow "Student 5 CI". |
...
| 13 | Browser evidence for the report | TL-FR-01..09 | **Outstanding** | **TODO: attach application screenshots** - `http://localhost:5105/` after `docker compose up --build student5-frontend student5-backend student5-database ollama ollama-model-setup`. |
