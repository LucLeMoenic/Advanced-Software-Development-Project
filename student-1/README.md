...
The componentised traveller frontend provides search, result states, provider-import and fallback notices, and history reopen/rename/delete through the backend API. The backend now imports an uncached destination from LiteAPI, validates the official v3 rates response, stores accommodations only through the database API, re-queries eligible candidates, ranks them with Ollama, and persists the result. Its 61 backend tests, 7 frontend tests, and production build pass. The unified page opens the feature at `/accommodation/`; a replacement LiteAPI key and one real sandbox run are still required because the originally supplied key was disclosed and was not used or stored. Live application-model execution, manual 320/768/1280px browser checks, required accommodation records, a GitHub Actions run, and real local agentic-loop records remain open evidence work.
