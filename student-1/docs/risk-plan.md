...
| R-13 | Tables have fewer than the required 10 records. | 4 | 5 | 20 | The tracked SQLite database now contains 10 Search records created once through the API without runtime seed code. Mitchell must still create at least 10 valid Accommodation records through the functional application. | A count below 10 in either submitted table blocks CI/demo readiness and FR-16 completion. | Mitchell | Tracked database, API count, and application-created accommodation evidence. |
