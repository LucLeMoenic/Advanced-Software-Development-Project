...
|---|---|---|---|
| 1. Project Setup | Standard folder; three services; populated tables; Dockerfiles; shared route/theme; model config; workflow; Compose entries. | In progress | Standard folder, service projects, Dockerfiles, workflow, and Compose entries implemented; populated tables and shared route/theme integration remain |
| 2. Service Implementation | Healthy Vue frontend, ASP.NET backend, and ASP.NET/EF Core database API containers communicating over HTTP. | In progress | Chunk 1 service roots and health endpoints plus database catalogue CRUD/filtering are implemented under `database/`; endpoints use a scoped repository instead of EF directly; 12/12 database tests pass; search and recommendation HTTP behaviour remains |
...
| 7. Docker Compose | One stack with service DNS, health, shared Ollama/model setup, and SQLite volume. | In progress | `docker-compose.yml` contains all three accommodation services, health ordering, service DNS configuration, and `student1-sqlite-data`; full integrated stack evidence remains |
